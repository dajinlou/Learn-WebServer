...
int RequestData::analysisRequest()
{
    ...
    if (method == METHOD_POST)
    {
        ...
        // sprintf(header, "%s\r\n", header);
        header += string("Content-length: ") + to_string(send_content.length()) + "\r\n\r\n";
...
void RequestData::linkTimer(shared_ptr<MyTimer> mtimer)
{
    timer = mtimer;
}
...
void RequestData::handleRead()
{
    do
    {
        ...
        {
            // 有请求出现但读不到数据，可能是Request Aborted, 或者来自网络的数据没有到达等原因
            // 最可能对端已经关闭了，同一按照对端已经关闭处理
            error = true;
            break;
        }
    ...
    if (!error)
    {
        ...
        {
            debug() << "keep_alive=" << keep_alive << endl;
            if (keep_alive)
            {
                this->reset();
                events |= EPOLLIN;
...
void RequestData::handleConn()
{
    if (!error)
    {
        if (events != 0)
        {
            ...
            if ((events & EPOLLIN) && (events & EPOLLOUT)) //有读有写，保留读
            {
                events = __uint32_t(0);
                events |= EPOLLOUT;
            }
            ...
            {
                debug() << "Epoll::epoll_mod error" << endl;
            }
        }
        else if (keep_alive)
        {
            events |= (EPOLLIN | EPOLLET | EPOLLONESHOT);
            int timeout = 5 * 60 * 100;
            isAbleRead = false;
            isAbleWrite = false;
            Epoll::add_timer(shared_from_this(), timeout);
            __uint32_t _events = events;
